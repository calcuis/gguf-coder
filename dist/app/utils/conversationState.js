export class ConversationStateManager {
     ...
     */
    updateAfterToolExecution(toolCall, result) {
        if (!this.state)
            return;
     ...
     */
    detectRepetition(toolCall) {
        if (!this.state || this.state.recentToolCalls.length < 2)
            return false;
        const recentToolCalls = this.state.recentToolCalls;
        const recent = recentToolCalls.slice(-2);
        return recent.some(prevCall => prevCall.function.name === toolCall.function.name &&
     ...
     */
    describeToolAction(toolCall, _result) {
        ...
        switch (toolName) {
            ...
            case 'execute_bash': {
                ...
                return `Executed command: ${commandStr.substring(0, 50)}${commandStr.length > 50 ? '...' : ''}`;
            }
            default:
