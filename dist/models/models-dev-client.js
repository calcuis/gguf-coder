 ...
 */
import { TIMEOUT_HTTP_BODY_MS, TIMEOUT_HTTP_HEADERS_MS } from '../constants.js';
 ...
 */
const OLLAMA_MODEL_CONTEXT_LIMITS = {
    // Llama 3.2 models
    'llama3.2': 128000,
    'llama3.2:1b': 128000,
    'llama3.2:3b': 128000,
 ...
 */
function extractOllamaModelBase(modelName) {
    ...
    if (lower.includes('gemma'))
        return 'gemma';
    if (lower.includes('command-r-plus'))
        return 'command-r-plus';
    if (lower.includes('command-r'))
        return 'command-r';
    if (lower.includes('deepseek-coder-v2'))
 ...
 */
function getOllamaFallbackContextLimit(modelName) {
...
async function fetchModelsData() {
    try {
        ...
        if (response.statusCode !== 200) {
            throw new Error(`Failed to fetch models data: HTTP ${response.statusCode}`);
        }
...
async function findModelByName(modelName) {
    ...
    for (const [_providerId, provider] of Object.entries(data)) {
        for (const [_modelId, model] of Object.entries(provider.models)) {
            if (model.id.toLowerCase().includes(lowerName) ||
 ...
 */
export async function getModelContextLimit(modelId) {
    ...
    const normalizedModelId = modelId.endsWith(':cloud') || modelId.endsWith('-cloud')
...
//# sourceMappingURL=models-dev-client.js.map