...
async function createTempDir() {
    const tempDir = path.join(process.cwd(), '.test-temp', `checkpoint-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
test.serial('CheckpointManager creates checkpoints directory on save', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
        await manager.saveCheckpoint('test-checkpoint', messages, 'TestProvider', 'test-model');
...
test.serial('CheckpointManager saves checkpoint with provided name', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
        const metadata = await manager.saveCheckpoint('my-checkpoint', messages, 'TestProvider', 'test-model');
...
test.serial('CheckpointManager generates timestamp-based name when not provided', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
        const metadata = await manager.saveCheckpoint(undefined, messages, 'TestProvider', 'test-model');
...
test.serial('CheckpointManager saves correct metadata', async (t) => {
    ...
    try {
        ...
        const metadata = await manager.saveCheckpoint('test', messages, 'MyProvider', 'my-model');
...
test.serial('CheckpointManager throws error for duplicate checkpoint name', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
...
test.serial('CheckpointManager throws error for invalid checkpoint name', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
        await t.throwsAsync(async () => {
            await manager.saveCheckpoint('invalid/name', messages, 'Provider', 'model');
...
test.serial('CheckpointManager throws error loading non-existent checkpoint', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        await t.throwsAsync(async () => {
...
test.serial('CheckpointManager lists checkpoints sorted by timestamp (newest first)', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
...
test.serial('CheckpointManager returns empty list when no checkpoints', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const list = await manager.listCheckpoints();
...
test.serial('CheckpointManager throws error deleting non-existent checkpoint', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        await t.throwsAsync(async () => {
...
test.serial('CheckpointManager detects invalid checkpoint (missing metadata)', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        // Create a checkpoint directory without metadata
        const checkpointDir = path.join(tempDir, '.coder', 'checkpoints', 'broken');
...
test.serial('CheckpointManager checkpointExists returns true for existing checkpoint', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
...
test.serial('CheckpointManager checkpointExists returns false for non-existing checkpoint', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
...
test.serial('CheckpointManager getCheckpointMetadata returns metadata', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(5);
        await manager.saveCheckpoint('meta-test', messages, 'MetaProvider', 'meta-model');
...
test.serial('CheckpointManager generates description from first user message', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = [
        ...
        const metadata = await manager.saveCheckpoint('desc-test', messages, 'Provider', 'model');
...
test.serial('CheckpointManager truncates long descriptions', async (t) => {
    ...
    try {
        ...
        const metadata = await manager.saveCheckpoint('long-desc', messages, 'Provider', 'model');
...
test.serial('CheckpointManager handles empty messages for description', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = [
        ...
        const metadata = await manager.saveCheckpoint('no-user', messages, 'Provider', 'model');
...
test.serial('CheckpointManager loadCheckpoint with validateIntegrity option', async (t) => {
    const tempDir = await createTempDir();
    try {
        const manager = new CheckpointManager(tempDir);
        const messages = createMockMessages(2);
        await manager.saveCheckpoint('validate-test', messages, 'Provider', 'model');
...
//# sourceMappingURL=checkpoint-manager.spec.js.map