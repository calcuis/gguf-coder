...
async function createTempDir() {
    const tempDir = path.join(process.cwd(), '.test-temp', `snapshot-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
async function createTestFile(dir, relativePath, content) {
...
test.serial('FileSnapshotService captures files in subdirectories', async (t) => {
    ...
    try {
        ...
        await createTestFile(tempDir, 'src/utils/helper.ts', 'export function help() {}');
        const service = new FileSnapshotService(tempDir);
        const snapshots = await service.captureFiles([
            'src/index.ts',
            'src/utils/helper.ts',
...
test.serial('FileSnapshotService handles non-existent files gracefully', async (t) => {
    const tempDir = await createTempDir();
    try {
...
test.serial('FileSnapshotService overwrites existing files on restore', async (t) => {
    const tempDir = await createTempDir();
    try {
...
test.serial('FileSnapshotService getSnapshotSize calculates correct size', async (t) => {
...
test.serial('FileSnapshotService getSnapshotSize handles empty snapshots', async (t) => {
    const service = new FileSnapshotService(process.cwd());
    const snapshots = new Map();
    const size = service.getSnapshotSize(snapshots);
    t.is(size, 0);
});
test.serial('FileSnapshotService getSnapshotSize handles unicode content', async (t) => {
    const service = new FileSnapshotService(process.cwd());
    const snapshots = new Map();
...
test.serial('FileSnapshotService validateRestorePath validates writable paths', async (t) => {
    const tempDir = await createTempDir();
    try {
        const service = new FileSnapshotService(tempDir);
        const snapshots = new Map();
...
test.serial('FileSnapshotService validateRestorePath validates existing writable files', async (t) => {
    const tempDir = await createTempDir();
    try {
...
test.serial('FileSnapshotService validateRestorePath creates missing directories', async (t) => {
    const tempDir = await createTempDir();
    try {
        const service = new FileSnapshotService(tempDir);
        const snapshots = new Map();
...
test.serial('FileSnapshotService validateRestorePath handles non-writable directories', async (t) => {
    const tempDir = await createTempDir();
    try {
        ...
        t.true(result.errors.some(error => error.includes('Cannot create directory') ||
...
test.serial('FileSnapshotService validateRestorePath handles non-writable existing files', async (t) => {
    const tempDir = await createTempDir();
    try {
        ...
        t.false(result.valid);
        t.true(result.errors.length > 0);
        t.true(result.errors.some(error => error.includes('Cannot write to file')));
    }
    finally {
        // Restore permissions for cleanup
        try {
...
test.serial('FileSnapshotService validateRestorePath skips file checks when directory creation fails', async (t) => {
    const tempDir = await createTempDir();
    try {
        ...
        const dirErrors = result.errors.filter(error => error.includes('Cannot create directory'));
...
test.serial('FileSnapshotService validateRestorePath handles multiple files with mixed scenarios', async (t) => {
    const tempDir = await createTempDir();
    try {
...
test.serial('FileSnapshotService validateRestorePath handles deeply nested paths', async (t) => {
    const tempDir = await createTempDir();
    try {
        const service = new FileSnapshotService(tempDir);
        const snapshots = new Map();
        snapshots.set('level1/level2/level3/level4/file.txt', 'Deeply nested content');
        const result = await service.validateRestorePath(snapshots);
        t.true(result.valid);
        t.is(result.errors.length, 0);
        // All nested directories should be created
        const nestedDir = path.join(tempDir, 'level1/level2/level3/level4');
...
test.serial('FileSnapshotService captures files with special characters in content', async (t) => {
    const tempDir = await createTempDir();
    try {
...
//# sourceMappingURL=file-snapshot.spec.js.map