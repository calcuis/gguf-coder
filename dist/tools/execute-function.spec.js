...
test('execute_bash tool works correctly', async (t) => {
    const result = (await executeBashTool.tool.execute({ command: 'echo "test"' }, {
        toolCallId: 'test-1',
        messages: [],
    }));
    t.truthy(result);
...
test('execute_bash includes exit code', async (t) => {
    const result = (await executeBashTool.tool.execute({ command: 'echo "success"' }, {
        toolCallId: 'test-2',
        messages: [],
    }));
    t.truthy(result);
...
test('execute_bash captures stderr', async (t) => {
    const result = (await executeBashTool.tool.execute({ command: 'echo "error" >&2' }, {
        toolCallId: 'test-3',
        messages: [],
    }));
    t.truthy(result);
...
test('read_file reads existing file', async (t) => {
    ...
    try {
        const result = (await readFileTool.tool.execute({ path: testFile }, {
            toolCallId: 'test-4',
            messages: [],
        }));
        t.truthy(result);
...
test('read_file handles non-existent file', async (t) => {
    ...
    await t.throwsAsync(async () => {
        await readFileTool.tool.execute({ path: nonExistentFile }, {
            toolCallId: 'test-5',
            messages: [],
        });
...
test('read_file reads with line ranges', async (t) => {
    ...
    try {
        const result = (await readFileTool.tool.execute({ path: testFile, start_line: 2, end_line: 4 }, {
            toolCallId: 'test-6',
            messages: [],
        }));
        t.truthy(result);
...
//# sourceMappingURL=execute-function.spec.js.map