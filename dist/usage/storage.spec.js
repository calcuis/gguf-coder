...
import { addSession, clearUsageData, getLastNDaysAggregate, getTodayAggregate, readUsageData, writeUsageData, } from './storage.js';
console.log('\nstorage.spec.ts');
// ============================================================================
// Test Setup
// ============================================================================
...
function createTestDir() {
    const testDir = path.join(os.tmpdir(), `coder-test-${Date.now()}-${Math.random().toString(36).substring(7)}`);
...
test('migration handles missing legacy config directory gracefully', t => {
    ...
    process.env.CODER_CONFIG_DIR = path.join(os.tmpdir(), 'coder-nonexistent');
...
function createMockBreakdown(total = 1000) {
    return {
...
function createMockSession(provider = 'test-provider', model = 'test-model', tokens = 1000) {
    return {
        id: `session-${Date.now()}`,
        timestamp: Date.now(),
        provider,
...
test('readUsageData returns empty data on read error', t => {
    // Write invalid JSON
    const dataHome = process.env.XDG_DATA_HOME || path.join(os.tmpdir(), 'test-data');
