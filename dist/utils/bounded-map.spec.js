...
test('BoundedMap - enforces max size limit', t => {
    const map = new BoundedMap({ maxSize: 3 });
    map.set('a', 1);
    map.set('b', 2);
    map.set('c', 3);
    t.is(map.size, 3);
...
test('BoundedMap - evicts oldest entry first', t => {
    const map = new BoundedMap({ maxSize: 2 });
    map.set('first', 1);
    map.set('second', 2);
    map.set('third', 3);
...
test('BoundedMap - works with different key/value types', t => {
    const numberMap = new BoundedMap();
    numberMap.set(1, 'one');
    numberMap.set(2, 'two');
    t.is(numberMap.get(1), 'one');
    const objectMap = new BoundedMap();
...
test('BoundedMap - no TTL when ttl is 0 or undefined', async (t) => {
    const map1 = new BoundedMap({ ttl: 0 });
    const map2 = new BoundedMap({ ttl: undefined });
    map1.set('a', 1);
    map2.set('b', 2);
...
//# sourceMappingURL=bounded-map.spec.js.map