...
test('deepwiki template: builds correct HTTP config', t => {
    ...
    t.is(config.description, 'DeepWiki provides up-to-date documentation you can talk to, for every repo in the world.');
...
test('sequential-thinking template: builds correct HTTP config', t => {
    ...
    t.is(config.timeout, 30000);
    t.is(config.description, 'Dynamic and reflective problem-solving through thought sequences.');
...
test('context7 template: builds correct HTTP config', t => {
    ...
    t.is(config.timeout, 30000);
    t.is(config.description, 'Up-to-date code documentation for LLMs and AI code editors.');
...
test('remote-fetch template: builds correct HTTP config', t => {
    ...
    t.is(config.timeout, 30000);
    t.is(config.description, 'Web content fetching and conversion for efficient LLM usage');
...
test('github-remote template: builds correct HTTP config with headers', t => {
    ...
    t.is(config.timeout, 30000);
    t.is(config.description, 'Remote GitHub MCP server for repository management and operations');
...
test('remote templates: have no required fields', t => {
    ...
    for (const templateId of remoteTemplates) {
        ...
        const config = template.buildConfig({});
        t.truthy(config, `Remote template ${templateId} should build with empty answers`);
...
test('all templates: include transport field', t => {
    for (const template of MCP_TEMPLATES) {
        ...
        t.truthy(config.transport, `Template ${template.id} missing transport field`);
        // Test that transport is a valid type
        const validTransports = ['stdio', 'websocket', 'http'];
        t.true(validTransports.includes(config.transport), `Template ${template.id} has invalid transport: ${config.transport}`);
    }
});
test('local templates: use stdio transport', t => {
    ...
    for (const templateId of localTemplates) {
        ...
        const config = template.buildConfig(answers);
        t.is(config.transport, 'stdio', `Template ${templateId} should use stdio transport`);
    }
});
test('remote templates: use http transport', t => {
    ...
    for (const templateId of remoteTemplates) {
        ...
        t.truthy(template, `Remote template ${templateId} not found`);
        const config = template.buildConfig({});
        t.is(config.transport, 'http', `Remote template ${templateId} should use http transport`);
    }
});
// ============================================================================
...
// Tests for Transport Field (New Feature)
// ============================================================================
//# sourceMappingURL=mcp-templates.spec.js.map