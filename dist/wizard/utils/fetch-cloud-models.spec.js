...
function createCloudModelsResponse(models) {
    return { data: models };
}
// Helper to create GitHub-style response (direct array)
function createGitHubResponse(models) {
    return models;
}
...
test.serial('fetchCloudModels: Anthropic - uses correct endpoint and headers', async (t) => {
    const { mockFetch, getCapturedUrl, getCapturedHeaders } = createMockFetch(createCloudModelsResponse([{ id: 'model1' }]));
    globalThis.fetch = mockFetch;
    try {
...
test.serial('fetchCloudModels: GitHub - uses correct endpoint and headers', async (t) => {
    const { mockFetch, getCapturedUrl, getCapturedHeaders } = createMockFetch(createGitHubResponse([{ id: 'model1', name: 'Model 1' }]));
    globalThis.fetch = mockFetch;
    try {
...
//# sourceMappingURL=fetch-cloud-models.spec.js.map