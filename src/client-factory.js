...
export async function createLLMClient(provider) {
    // Check if coder.config.json exists
    const agentsJsonPath = getClosestConfigFile('coder.config.json');
...
async function createAISDKClient(requestedProvider, hasConfigFile = true) {
    // Load provider configs
    const providers = loadProviderConfigs();
    if (providers.length === 0) {
        const configPath = getClosestConfigFile('coder.config.json');
        const cwd = process.cwd();
        const isInCwd = configPath.startsWith(cwd);
        const cwdPath = !isInCwd ? join(cwd, 'coder.config.json') : undefined;
        if (!hasConfigFile) {
            throw new ConfigurationError('No coder.config.json found', configPath, cwdPath, false);
        }
        else {
            throw new ConfigurationError('No providers configured in coder.config.json', configPath, cwdPath, true);
        }
    }
